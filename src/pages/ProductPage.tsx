...
export default function ProductPage() {
  ...
  // Reset state when product or color changes
  useEffect(() => {
  ...
  // Prefer DB variants; fall back to legacy product.colors / images
  ...
  const activeImages = currentColor?.images?.length ? currentColor.images : product?.images || [];

  // Sizes available for selected color (from variants), else legacy product.sizes
  const sizeOptions = useMemo(() => {
    ...
    return (product?.sizes || []).map((s) => ({ size: s, stock: product?.stock ?? 0 }));
  ...
  const handleAddToCart = () => {
    const size = selectedSize || sizeOptions[2]?.size || sizeOptions[0]?.size || "M";
    ...
    <>
      ...
      <main className="pt-20 md:pt-24 pb-24 bg-background min-h-screen">
        <div className="container mx-auto px-6">
          {/* Breadcrumb */}
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex items-center gap-2 text-xs text-muted-foreground mb-8">
            <Link to="/" className="hover:text-foreground transition-colors flex items-center gap-1"><ChevronLeft size={12} /> Back</Link>
            <span>/</span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20">
              ...
              <div className="flex gap-3 flex-wrap">
                {activeImages.map((img, i) => (
                  <button
                    ...
                    className={`w-20 h-24 overflow-hidden bg-muted border-2 transition-colors ${selectedImage === i ? "border-foreground" : "border-transparent"}`}
                  >
            ...
            {/* Info */}
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6, delay: 0.1 }} className="lg:sticky lg:top-28 lg:self-start">
                  ...
                  <div className="flex gap-3">
                    {colors.map((color, i) => (
                      <button
                        ...
                        title={color.name}
                        className={`w-8 h-8 rounded-full border-2 transition-all ${selectedColorIdx === i ? "border-foreground scale-110" : "border-border"}`}
              ...
              <div className="mb-8">
                ...
                <div className="inline-flex items-center border border-border">
                  <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="w-12 h-12 flex items-center justify-center hover:bg-muted">
                    <Minus size={16} />
                  </button>
                  <span className="w-12 h-12 flex items-center justify-center text-sm font-semibold border-x border-border">{quantity}</span>
                  <button onClick={() => setQuantity((q) => q + 1)} className="w-12 h-12 flex items-center justify-center hover:bg-muted">
                    <Plus size={16} />
                  </button>
                </div>
              </div>
              ...
              <div className="flex gap-3">
                <motion.button
                  ...
                  disabled={sizeOptions.length > 0 && currentVariantStock <= 0}
                  className={`flex-1 flex items-center justify-center gap-2 py-4 text-sm font-semibold tracking-widest uppercase transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed ${
                    addedFeedback ? "bg-green-600 text-primary-foreground" : "bg-foreground text-primary-foreground hover:bg-foreground/90"
                  }`}
                >
                  <ShoppingBag size={16} />
                  {addedFeedback ? "Added!" : sizeOptions.length > 0 && currentVariantStock <= 0 ? "Out of stock" : "Add to Cart"}
                </motion.button>

                <motion.button whileTap={{ scale: 0.9 }} onClick={() => toggle(product.id)} className="w-14 h-14 flex items-center justify-center border border-border hover:border-foreground">
